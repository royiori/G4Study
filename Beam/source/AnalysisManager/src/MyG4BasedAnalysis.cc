...
void MyG4BasedAnalysis::BeginOfRunAction()
{
    ...
    */
    ...
    analysisManager->CreateNtuple("Hit", "Hits"); // ntuple Id = 1
    analysisManager->CreateNtupleDColumn("X");
    analysisManager->CreateNtupleDColumn("Y");
    analysisManager->CreateNtupleDColumn("Z");
...
void MyG4BasedAnalysis::SteppingAction(const G4Step *aStep)
{
    ...
    */
    ...
    G4LogicalVolume *presentVolume = pVolume->GetLogicalVolume();
    if (presentVolume->GetName() == "FeVol") //打到阳极板上的才记录
        return;

    //if (!aStep->IsFirstStepInVolume()) //Det里只有第一个step才保存，避免在FR4里有多次step保存多次的情况
    //    return;
    ...
    analysisManager->FillNtupleDColumn(1, 3, postStepPoint->GetTotalEnergy());
    analysisManager->FillNtupleDColumn(1, 4, double(pdgID));
